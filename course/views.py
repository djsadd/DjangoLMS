...
@login_required
def electronic_library(request):
    query = request.GET.get("q", "").strip()
    resources = ElectronicResource.objects.all()
    if query:
    ...
    return render(
        request,
        "course/electronic_library.html",
        {
        "title": _("Electronic Library"),
...
@login_required
def electronic_library_manage(request):
    if not request.user.is_superuser:
        messages.error(request, _("You are not allowed to manage library items."))
        return redirect("electronic_library")
    ...
    if query:
        resources = resources.filter(Q(title__icontains=query) | Q(description__icontains=query))
    if kind == "files":
        resources = resources.filter(file__isnull=False).exclude(file="")
    elif kind == "links":
        resources = resources.filter(link__isnull=False).exclude(link="")

    if request.method == "POST":
        form = ElectronicResourceForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            messages.success(request, _("Electronic resource has been created."))
            return redirect("electronic_library_manage")
    ...
    return render(
        request,
        "course/electronic_library_manage.html",
        {
        "title": _("Manage Electronic Library"),
        "resources": resources_page,
        "form": form,
        "query": query,
        "kind": kind,
