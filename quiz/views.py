...
def check_program_student(request, pk):
    course = Course.objects.get(pk=pk)
    programs = Program.objects.filter(course=course)
    di = []
    for program in programs:
        programs_code = ProgramCode.objects.filter(program=program)
        for res in programs_code:
            if res.pass_mark!=None:
                continue
            di.append(res)
    print(di)
    context = {"di": di}
    return render(request, template_name="quiz/studen_code_list.html", context=context)


def detail_check_code(request, pk):
    if request.method=="POST":
        ...
        if action == "save":
            ...
            program_code = ProgramCode.objects.get(pk=pk)
            program_code.pass_mark = grade
            program_code.save()
            context = {"program_code": program_code, "program": program_code.program}
            return render(request, template_name="quiz/student_check_code.html", context=context)
        else:
            return execute_python_code(request, pk) 
    program_code = ProgramCode.objects.get(pk=pk)
    context = {"program_code": program_code, "program": program_code.program}
    return render(request, template_name="quiz/student_check_code.html", context=context)

@csrf_exempt
def execute_python_code(request, pk):
    ...
    if request.method == "POST":
        ...
        if action=="save":
            ...
            program_code = ProgramCode.objects.create(student=request.user, program=program, code=code)
            taken = TakenCourse.objects.get(student=student, course=program.course)
            print(taken)
            program_code.grade = taken
            program_code.save()
        try:
            data = json.loads(request.body)
            code = data.get("code", "")
            ...
            print("Отправляем запрос в API:", json.dumps(payload, indent=4))
...
def code_editor(request, pk):
    ...
    program_code = ProgramCode.objects.filter(student=request.user, program=program)
    if program_code:
        program_code = program_code.first()
        return render(request, "quiz/execute_code.html", context={'program': program, "program_code" : program_code})
    return render(request, "quiz/execute_code.html", context={'program': program})


def view_code(request, pk):
    print(ProgramCode.objects.get(student=request.user, program=program, code=None))
    return render(request, "quiz/execute_code.html", context={'program': program})
